...
static int t9p_rtems_fs_symlink(const rtems_filesystem_location_info_t* parentloc, const char* name, size_t namelen, const char* target);
static int t9p_rtems_fs_utimenfs(const rtems_filesystem_location_info_t* loc, struct timespec times[2]);
static int t9p_rtems_fs_rmnod(const rtems_filesystem_location_info_t* parentloc, const rtems_filesystem_location_info_t* loc);
static int t9p_rtems_fs_mknod(const rtems_filesystem_location_info_t* parentloc, const char* name, size_t namelen, mode_t mode, dev_t dev);
static bool t9p_rtems_fs_are_nodes_equal(const rtems_filesystem_location_info_t* a, const rtems_filesystem_location_info_t* b);
...
static int t9p_rtems_fs_link(const rtems_filesystem_location_info_t*, const rtems_filesystem_location_info_t*, const char*, size_t);
static long t9p_rtems_fs_readlink(const rtems_filesystem_location_info_t* loc, char* buf, size_t bufsize);
static int t9p_rtems_fs_rename(const rtems_filesystem_location_info_t*, const rtems_filesystem_location_info_t*,
                               const rtems_filesystem_location_info_t*, const char*, size_t);
static int t9p_rtems_fs_chown(const rtems_filesystem_location_info_t* loc, uid_t owner, gid_t group);
...
static rtems_filesystem_eval_path_generic_status t9p_rtems_eval_path_token(rtems_filesystem_eval_path_context_t* ctx, void* arg,
...
static int rtems_mode_to_t9p(mode_t mode) {
    int m = 0;
...
static int t9p_rtems_fsmount_me(rtems_filesystem_mount_table_entry_t* mt_entry, const void *data) {
    /*
mount -t 9p -o uid=1000,gid=1000,ip=10.0.2.2:10002,port=10002,user=jeremy /home/jeremy/dev/lw9p/tests/fs /test
    */
    TRACE
    if (!data) {
        errno = EINVAL;
    ...
    fi->c = t9p_init(&t, &fi->opts.opts, mt_entry->dev, ip, mt_entry->target);
    if (fi->c == NULL)
        return -1;
...
t9p_rtems_eval_path_token(rtems_filesystem_eval_path_context_t* ctx, void* arg, const char* token, size_t tokenlen)
{
    TRACE
...
static int t9p_rtems_fs_symlink(const rtems_filesystem_location_info_t *parentloc, 
    const char *name, size_t namelen, const char *target)
{
    t9p_rtems_fs_info_t* fi = parentloc->mt_entry->fs_info;
    t9p_rtems_node_t* n = parentloc->node_access;
...
static int t9p_rtems_fs_rmnod(const rtems_filesystem_location_info_t *parentloc,
    const rtems_filesystem_location_info_t *loc)
{
    return -1;
}

static int t9p_rtems_fs_mknod(const rtems_filesystem_location_info_t *parentloc,
...
static void t9p_rtems_fs_freenode(const rtems_filesystem_location_info_t *loc) {
    ...
    t9p_rtems_node_t* n = loc->node_access;
...
static int t9p_rtems_fs_link(const rtems_filesystem_location_info_t *parentloc,
...
static int t9p_rtems_fs_rename(const rtems_filesystem_location_info_t *oldparentloc, const rtems_filesystem_location_info_t *oldloc,
